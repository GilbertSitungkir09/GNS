...
conn.browserDescription = ['WhatsApp Bot', 'Chrome', '3.0']
    ...
    conn.on('qr', async qr => {
      let scan = await parent.sendFile(m.chat, await qrcode.toDataURL(qr, { scale: 20 }), 'qrcode.png', 'Scan QR ini untuk jadi bot sementara\n\n1. Klik titik tiga di pojok kanan atas\n2. Ketuk WhatsApp Web\n3. Scan QR ini \nQR Expired dalam 20 detik', m)
      setTimeout(() => {
      ...
      setTimeout(async () => {
        try {
          ...
          if (conn.user && conn.user.jid)
            parent.sendMessage(conn.user.jid, `Koneksi Terputus Dari Server Bot...`, MessageType.extendedText)
          let i = global.conns.indexOf(conn)
          if (i < 0) return
          delete global.conns[i]
          global.conns.splice(i, 1)
