...
let handler = async(m, { conn, text }) => {
    ...
    if (!text) return conn.reply(m.chat, 'Masukan Nama Daerahnya Syg', m)

	axios.get(`https://tobz-api.herokuapp.com/api/jadwalshalat?q=${text}&apikey=Tobzzz17`).then ((res) => {
...
handler.help = ['jadwalsholat'].map(v => v + ' <daerah>')
handler.tags = ['quran']
